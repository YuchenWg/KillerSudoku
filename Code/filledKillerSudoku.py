    ...
    def genCages(self, method='rw', maxCageSize=5):
        if method == 'rw':
            ...
            while self.notAssigned:  # cell: [x1,y1]
                ...
                while walkDirectionList != [] and self.cageSize[cageInd] < maxCageSize:
                    ...
                    if self.nextCell[0] < 0 or self.nextCell[0] > 8 or self.nextCell[1] < 0 or self.nextCell[
                        1] > 8 \
                ...
                # if no available direction or cage is full, go to next not assigned cell
                cageInd += 1
                self.notAssigned.remove(self.currentCell)
